javascript:(function() {
    ...
    async function renderHtmlToCanvas(html, canvas, ctx) {
    ...
    function init() {
      ...
      if (!document.getElementById('note-modal')) {
        ...
        captureBtn.addEventListener('click', async () => {
          ...
          if (selectionHTML) {
            const tempCanvas = await renderHtmlToCanvas(selectionHTML, canvas, ctx);
            const aspectRatio = tempCanvas.width / tempCanvas.height;
            const containerWidth = modal.clientWidth;
            const containerHeight = modal.clientHeight - 60; 
            ...
            if (newHeight + 60 > modalHeight) {
              modal.style.height = `${newHeight + 60}px`;
            }
          }
        });
